...
const ifHotels = [
  ...
  const totalCancellation = bookingData.filter((record:any) => {
    ...
    return (
      <>
      <div className=" max-h-[800px] overflow-y-scroll lg:max-w-[90%] absolute p-6 items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row  dark:border-gray-700 dark:bg-gray-800 w-full">
      <div className="w-full">
        <h1 className="text-center mx-auto text-xl font-bold my-2">{variable}</h1>
        <button>
        <FaTimes className="absolute top-2 right-2 text-2xl text-gray-500 hover:text-gray-700 cursor-pointer" onClick={() => onClose(false)} />
        </button>
      <div className="w-full relative overflow-x-auto shadow-md sm:rounded-lg cursor-pointer">
        <table className="p-4 w-full border-white border-2 text-sm text-left text-gray-500  dark:bg-inherit  dark:text-gray-400">
