...
function TodaysModifiedBooking() {
  ...
  const thisWeekModifications: BookingData[] = calculateThisWeekModification(
    bookingData,
    startOfWeek,
    endOfWeek,
  );
  const thisWeekModification: number = thisWeekModifications.length;
  ...
  thisWeekModifications.forEach((record) => {
    const ModifiedDate: Date = new Date(record.updatedAt);
  ...
  const TodaysModifiedBooking = {
    ...
    dataKey: "users",
    percentage: thisWeekModification,
